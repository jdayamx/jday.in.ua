...
class Metadata extends CApplicationComponent {
    ...
    */
    ...
    public function getActionsDesc($controller, $module=null)
    {
        if ($module!=null){
            $path=join(DIRECTORY_SEPARATOR,array(Yii::app()->modulePath,$module,'controllers'));
            $this->setModuleIncludePaths($module);
        ...
        foreach($methods as $method)
        {
            if (strpos($method->name,'action')===0 and ctype_upper($method->name[6]))
            {
                ...
                $actions[str_replace('action','',$method->name)] = $method->getDocComment();
            }
        }
        return $actions;
    ...
    */
    ...
    public function getModelsDesc($module=null,$include_classes=false)
    {
        ...
        foreach ($files as $f)
        {
            if (stripos($f,'.php')!==false)
            {
            	$reflection = new ReflectionClass(str_ireplace('.php','',$f));
            	$desc = trim(str_replace(array('/**','*/'),'',$reflection->getDocComment()));
                $models[str_ireplace('.php','',$f)] = ($desc?$desc:str_ireplace('.php','',$f));
                if ($include_classes)
                {
                    include_once($path.DIRECTORY_SEPARATOR.$f);
                }
