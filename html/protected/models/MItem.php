...
class MItem extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, name, reg_proc, timetoget, cost_by_craft, cost, count', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getCalcCost() {
		...
		if($ret===false)
		{
		    if($this->craft) {
				...
				$ret = round($this->cost*$this->count/$this->count,2);
			}
	...
	public function CalcStack($n) {
		...
		if($stack_count>0) {
			return $stack_count.'/'.($n-$stack_count*64);
		} else {
			return '0/'.$n;
		}
	}
	...
	public function getImage() {
		return CHtml::image('/img/minecraft/item/'.$item->id.'.png','');
	}

	//fooooooo all in module? ne lepi dalshe tut cod
