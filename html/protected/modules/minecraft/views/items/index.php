...
$this->pageTitle=Yii::app()->name . ' - minecraft items';
...
foreach(MItemCategory::model()->findAll() as $cat) {
	...
	foreach($cat->items(array('order'=>'id+0')) as $item) {
		...
		echo '<tr id="item'.$item->id.'">
				<td width="5%">'.str_replace('-',':',$item->id).'</td>
				<td  width="32">'.CHtml::link(CHtml::image('/img/minecraft/item/'.$item->id.'.png',''),array('/minecraft/items/view','item'=>$item->id)).'</td>
