...
HLMDL.prototype =
{
	...
	{
		var path = "/data/cstrike/"+filename;
	...
	readTexture: function(buffer, tex)
	{
		...
		for (var y=0; y<tex.height; y++)
		{
			...
			for (var x=0; x<tex.width; x++)
			{
                ...
                if(rgb[idx+1]==0 && rgb[idx]==0 && rgb[idx] == rgb[idx] && rgb[idx+1] == palette[766] && rgb[idx+2] ==palette[767]) {
                	alpha = 0;
                	data[index*4+0] = 0;
					data[index*4+1] = 0;
					data[index*4+2] = 0;
	...
	buildTexture: function(image, header, w, h, data, alpha)
	{
		...
		//gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
 		...
 		//}
 		//gl.bindTexture(gl.TEXTURE_2D,  createSolidTexture(gl, [0,0,0,255]));
 		gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, w, h, 0, gl.RGBA, gl.UNSIGNED_BYTE, data);
 		//gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, w,h, 0, gl.RGBA, gl.UNSIGNED_BYTE, data);
