...
HLMDL.prototype =
{
	...
	parse: function(buffer)
	{
		...
		//alert("textures: "+ this.header.numtextures);
	...
	readTexture: function(buffer, tex)
	{
		...
		var rgb = new Uint8Array(tex.width * tex.height * 4);
		//alert("R:"+palette[768-3]+" G:"+palette[768-2]+" B:"+palette[768-1]);
		for (var y=0; y<tex.height; y++)
		{
			...
			for (var x=0; x<tex.width; x++)
			{
                ...
                if((rgb[idx+1]==0 || rgb[idx+1]==255) && (rgb[idx]==0 || rgb[idx]==255) && rgb[idx] == rgb[idx] && rgb[idx+1] == palette[766] && rgb[idx+2] ==palette[767]) {
                	if(rgb[idx+1]==0 && rgb[idx]==0&&rgb[idx+2]==0) {
                		if(tex.name.match(/alpha/)) {
                			alpha = 0;
                		} else {
                			alpha = 0;
                		}
		...
		if (newWidth != tex.width || newHeight != tex.height)
		{
			...
			rgb = new Uint8Array(newWidth * newHeight * 4);
			for (var i=0; i<data.length/4; i++)
			{
				rgb[i*4] = data[i*4];
				rgb[i*4+1] = data[i*4+1];
				rgb[i*4+2] = data[i*4+2];
				rgb[i*4+3] = data[i*4+3];
	...
	buildTexture: function(image, header, w, h, data, alpha)
	{
		image.width = w;
		image.height = h;
	...
	calcBoneQuaternion: function(buffer, frame, s, bone, anim, q, animindex)
	{
		...
		for (var j=0; j<3; j++)
		{
			...
			else
			{
				...
				if(animvalue == undefined ) continue;
				...
				if (animvalue.valid > k)
				{
					...
					else
					{
						if (animvalue.total > k+1)
							...
							if(animvalue[animvalue.valid+2] != undefined)
	...
	draw: function(shader)
	{
		...
		//alert(shader.attribute.texCoord2);
