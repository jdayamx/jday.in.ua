...
HLBSP.prototype =
{
	...
	loadTextures: function(texInfos, callback)
	{
		...
		else
		{
			...
			{
				var path = "/data/textures/"+toLoad[i]+".png";
				var img = new Image();
				...
				{
					console.error(err);
					img.src = "/img/mask/transparent.png";
					return true;
				});
	...
	parseEntities: function(buffer)
	{
		...
		for (var i=0; i<entries.length; i++)
		{
			...
			for (var j=0; j<match.length/2; j++) {
				if(match[j*2] != undefined && match[j*2+1] != undefined) {
	...
	spawnEntities: function(entities, callback)
	{
		...
		for (var i=0; i<entities.length; i++)
		{
			...
			if( model_path != undefined && model_path.length >= 10 ) {
				e["classname"] = model_path;
				Entities[model_path] = { model: model_path };
				if(type == undefined) {
					var type = Entities[model_path];
				}
				type.model = model_path;
				//alert(Entities[name]);
				//alert(name+": "+model_path);
			} //else alert(name+": "+model_path);
		...
		for (var i=0; i<paths.length; i++)
		{
			...
			self.models[paths[i]] = new HLMDL(paths[i], function()
			{
				if (--counter == 0)
				{
	...
	drawEntities: function(shader)
	{
		...
		for (var i=0; i<this.entities.length; i++)
		{
			...
			mat4.rotateZ(modelViewMat, (e.angle-45)*Math.PI/2);
